...
namespace Sample03.E3SClient
{
	public class E3SQueryClient
	{
		...
		public IEnumerable<T> SearchFTS<T>(string query, int start = 0, int limit = 50) where T : E3SEntity
		{
		...
		public IEnumerable SearchFTS(Type type, string query, int start = 0, int limit = 50)
		{
			HttpClient client = CreateClient();
			var requestGenerator = new FTSRequestGenerator(BaseAddress);
